package destinychar.powers;

import com.megacrit.cardcrawl.actions.AbstractGameAction;
import com.megacrit.cardcrawl.actions.common.DamageAction;
import com.megacrit.cardcrawl.actions.common.RemoveSpecificPowerAction;
import com.megacrit.cardcrawl.actions.defect.DamageAllButOneEnemyAction;
import com.megacrit.cardcrawl.cards.DamageInfo;
import com.megacrit.cardcrawl.core.AbstractCreature;
import com.megacrit.cardcrawl.dungeons.AbstractDungeon;
import com.megacrit.cardcrawl.powers.AbstractPower;

import static destinychar.DestinyCharMod.makeID;

public class Scorch extends BasePower{
    public static final String POWER_ID = makeID("Scorch");
    private static final AbstractPower.PowerType TYPE = AbstractPower.PowerType.DEBUFF;
    private static final boolean TURN_BASED = false;
    private int lastStack = -1;
    //The only thing TURN_BASED controls is the color of the number on the power icon.
    //Turn based powers are white, non-turn based powers are red or green depending on if their amount is positive or negative.
    //For a power to actually decrease/go away on its own they do it themselves.
    //Look at powers that do this like VulnerablePower and DoubleTapPower.

    public Scorch(AbstractCreature owner, int amount) {
        super(POWER_ID, TYPE, TURN_BASED, owner, amount);
        //this.flash();
        if(owner.hasPower(POWER_ID)){
            if(owner.getPower(POWER_ID).amount + amount >= 10) {
                int[] damage = new int[20];
                for(int i = 0; i < 20; i++){
                    damage[i] = 15;
                }
                addToBot(new DamageAction(owner, new DamageInfo(owner, 30, DamageInfo.DamageType.NORMAL), AbstractGameAction.AttackEffect.FIRE));
                addToBot(new DamageAllButOneEnemyAction(AbstractDungeon.player, owner, damage, DamageInfo.DamageType.NORMAL, AbstractGameAction.AttackEffect.FIRE));
                owner.getPower(POWER_ID).amount -= 10;
            }
        }
    }

    public void updateDescription() {
        this.description = DESCRIPTIONS[0];
    }

    public void atStartOfTurn() {
        if(lastStack == owner.getPower(POWER_ID).amount) {
            addToBot(new RemoveSpecificPowerAction(owner, owner, Scorch.POWER_ID));
        } else {
            lastStack = owner.getPower(POWER_ID).amount;
        }
    }
}
